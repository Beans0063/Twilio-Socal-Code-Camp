...
DataMapper.setup(:default, 'mysql://localhost/socalcodecamp')
...
"  <html><body><script src=""http://js.pusherapp.com/1.8/pusher.min.js""></script>
  ...
  <h1>SoCal Code Camp SMS messages " + APP_NUMBER + "</h1>
  <div id='messages'></div>
  </body></html>"
end
...
post '/github' do
  ...
  engineering_team_numbers.each do |number|
    payload['commits'].each do |sha, commit|
      send_sms(number,"[%s] %s - %s %s" % [ repository['name'], commit['author']['name'], commit['message'], commit['url'] ])
    end
  end
  "sent notifications"
end
